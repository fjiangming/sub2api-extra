...
“告警”页中通知通道右侧的测试按钮只验证固定测试消息能否送达。完整链路可以在“测试 -> 告警充值入口”中模拟，不需要修改余额快照或触发真实告警：

1. 选择一个已配置充值链接的供应商。所有供应商都会列出；未配置充值链接的供应商保留在列表中，但不能发送该项测试。
2. 选择接收测试消息的通知通道。这里是明确的人工测试，因此也允许选择当前停用的通道，不会向其他通道广播。
3. 保持“发送后打开移动端预览”开启并点击“发送测试告警”。系统发送通知后会自动打开约 `430×860` 的独立窗口；自动登录入口属于敏感能力，首次发送或上次验证超过 5 分钟时需要重新验证管理员身份。
4. 在手机上打开收到的 `[模拟测试]` 低余额提醒并点击“立即充值”或消息中的充值链接。
5. 页面按该供应商的真实配置执行：可用时签发一次性适配器登录入口；配置不完整、地址不安全或适配器不支持时发送原充值链接。测试页会显示实际采用的方式和降级原因。

桌面浏览器阻止自动弹窗时，可在发送结果右上角点击“打开移动端预览”。移动操作系统和微信不允许后台消息在没有用户操作时强制启动浏览器，因此微信侧的“自动打开”仍从点击通知中的充值链接开始。适配器入口遵守一次性票据规则：预览页不会消耗票据，确认继续后票据立即失效，再建立供应商会话并跳转充值页。

模拟测试不会创建 `alert_events`、不会触发自动化规则，也不会写入 `notification_deliveries`；会写入不含充值票据的安全审计记录。适配器自动登录测试会为微信通知和桌面移动预览分别签发一次性票据，两者互不影响，数据库仍只保存票据哈希。模拟余额优先使用供应商一级阈值的一半；未配置一级阈值时仅为消息展示使用模拟阈值 `20`，不会改写供应商配置。
...
│   │   └── custom.js           # 自定义适配器
│   ├── db/
│   │   └── index.js            # SQLite Schema（版本 14）与迁移
│   ├── http/
