...
仅提醒人工充值时，不需要部署外部充值服务，也不需要启用自动化写入开关。在供应商编辑页填写“余额预警值”“币种”和“充值链接”，再在告警页启用通知通道；账户余额低于阈值时，通知会附带该供应商的充值入口。企业微信通道使用群机器人 Webhook：配置 JSON 可填写 `{}`，凭据 JSON 填写 `{"webhookUrl":"https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=..."}`。普通个人微信可选择“Server酱（个人微信）”：配置 JSON 填写 `{}`，凭据 JSON 填写 `{"sendKey":"SCT..."}`。两种通道保存后都可使用“测试”按钮验证。
