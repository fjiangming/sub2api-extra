...
1. 填写“余额预警值”，例如 `20`。
2. 填写与余额快照一致的“币种”，例如 `USD`。
3. 填写“充值链接”，例如 `https://supplier.example/account/recharge`。
4. 保持“启用定时检查”并保存。

充值链接只接受 HTTP 或 HTTPS。链接会出现在微信通知中，不要在 URL 中放置一次性支付凭据、密码或长期有效 Token。

供应商级预警值会生成一条内置低余额规则：最新一次有效余额低于阈值即触发，持续低余额时每 `60` 分钟最多重复提醒一次。需要“连续多次低余额才提醒”或自定义冷却时间时，可以在“告警 -> 添加规则”中创建显式 `low_balance` 规则；此时应清空同一供应商的供应商级预警值，避免内置规则和显式规则重复通知。
...
| 导出方式 | 预警值与充值链接 | 显式告警规则 | 通知通道定义 | SendKey / Webhook 凭据 | 告警与投递历史 |
|---|:---:|:---:|:---:|:---:|:---:|
| 配置 JSON | 是 | 是 | 是 | 否 | 否 |
...
远端备份上传的是 SQLite 在线备份文件，因此会同时包含供应商预警值、充值链接、显式告警规则、通知通道、加密通知凭据、告警事件和投递历史。恢复数据库时必须同时保留原 `PROVIDER_MONITOR_SECRET`；缺少或更换该主密钥将无法解密数据库中的供应商和通知凭据。
