...
3. 点击“发送测试告警”。自动登录入口属于敏感能力，首次发送或上次验证超过 5 分钟时需要重新验证管理员身份。
4. 在手机上打开收到的 `[模拟测试]` 低余额提醒并点击“立即充值”或消息中的充值链接。
5. 页面按该供应商的真实配置执行：可用时签发一次性适配器登录入口；配置不完整、地址不安全或适配器不支持时发送原充值链接。测试页会显示实际采用的方式和降级原因。

移动操作系统和微信不允许后台消息在没有用户操作时强制启动浏览器，因此“自动打开”从点击通知中的充值链接开始。适配器入口仍遵守一次性票据规则：预览页不会消耗票据，确认继续后票据立即失效，再建立供应商会话并跳转充值页。

模拟测试不会创建 `alert_events`、不会触发自动化规则，也不会写入 `notification_deliveries`；会写入不含充值票据的安全审计记录。适配器自动登录测试仍会创建正常的一次性票据哈希。模拟余额优先使用供应商一级阈值的一半；未配置一级阈值时仅为消息展示使用模拟阈值 `20`，不会改写供应商配置。
