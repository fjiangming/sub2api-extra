...
const inputRegisterOnlyMode = document.getElementById('input-register-only-mode');
const registeredAccountsCount = document.getElementById('registered-accounts-count');
const registeredAccountsList = document.getElementById('registered-accounts-list');
const btnExportRegisteredAccounts = document.getElementById('btn-export-registered-accounts');
...
function applyAutoRunStatus(payload = currentAutoRun) {
  ...
  if (inputRegisterOnlyMode) {
    inputRegisterOnlyMode.disabled = settingsCardLocked;
  }
...
function applySettingsState(state) {
  ...
  if (inputRegisterOnlyMode) {
    inputRegisterOnlyMode.checked = Boolean(state?.registerOnlyMode);
  }
...
function normalizeRegisteredAccountStatus(status) {
  return status === 'callback_success' ? 'callback_success' : 'registered';
}

function getRegisteredAccounts(state = latestState) {
  ...
  return accounts
    ...
    .map((account) => ({
      ...
      status: normalizeRegisteredAccountStatus(account.status),
    }))
    .sort((a, b) => {
...
function getRegisteredAccountStatusLabel(account) {
  return normalizeRegisteredAccountStatus(account?.status) === 'callback_success'
    ? '回调成功'
    : '注册成功';
}

function getRegisteredAccountStatusClass(account) {
  return normalizeRegisteredAccountStatus(account?.status) === 'callback_success'
...
function renderRegisteredAccounts() {
  ...
  const callbackCount = accounts.filter((account) => normalizeRegisteredAccountStatus(account.status) === 'callback_success').length;

  if (registeredAccountsCount) {
    registeredAccountsCount.textContent = accounts.length
      ? `${accounts.length} 个账号，${callbackCount} 个回调成功`
      : '暂无账号';
  }
  if (btnExportRegisteredAccounts) {
    btnExportRegisteredAccounts.disabled = accounts.length === 0;
  }
  if (!registeredAccountsList) return;

  if (!accounts.length) {
    registeredAccountsList.innerHTML = '<div class="registered-empty">注册成功后会自动记录到这里。</div>';
    return;
  }

  registeredAccountsList.innerHTML = accounts.map((account) => `
    <div class="registered-account-item">
      <div class="registered-account-top">
        <div class="registered-account-email">${escapeHtml(account.email)}</div>
        <span class="registered-status-chip ${escapeHtml(getRegisteredAccountStatusClass(account))}">${escapeHtml(getRegisteredAccountStatusLabel(account))}</span>
      </div>
      <div class="registered-account-meta">
        <span>注册时间：${escapeHtml(formatAccountDateTime(account.registeredAt || account.createdAt))}</span>
        <span>回调时间：${escapeHtml(account.callbackAt ? formatAccountDateTime(account.callbackAt) : '未完成')}</span>
      </div>
    </div>
  `).join('');
}

function buildRegisteredAccountsExportFilename(date = new Date()) {
  const pad = (value) => String(value).padStart(2, '0');
  return `registered-accounts-${date.getFullYear()}${pad(date.getMonth() + 1)}${pad(date.getDate())}-${pad(date.getHours())}${pad(date.getMinutes())}${pad(date.getSeconds())}.txt`;
}

function exportRegisteredAccountsFile() {
  const accounts = getRegisteredAccounts();
  if (!accounts.length) {
    showToast('暂无可导出的注册账号。', 'warn', 1800);
    return;
  }

  const content = `${accounts.map((account) => account.email).join('\n')}\n`;
  const fileName = buildRegisteredAccountsExportFilename();
  downloadTextFile(content, fileName, 'text/plain;charset=utf-8');
  showToast(`已导出 ${accounts.length} 个注册账号`, 'success', 2200);
}
...
function updateButtonStates() {
  ...
  const registerOnlyMode = Boolean(latestState?.registerOnlyMode);

  for (let step = 1; step <= 9; step++) {
    const btn = document.querySelector(`.step-btn[data-step="${step}"]`);
    if (!btn) continue;

    if (!(registerOnlyMode && step > 5)) {
      btn.removeAttribute('title');
    }

    if (registerOnlyMode && step > 5) {
      btn.disabled = true;
      btn.title = '仅注册模式下会自动跳过该步骤';
...
btnExportRegisteredAccounts?.addEventListener('click', () => {
  exportRegisteredAccountsFile();
});
...
inputRegisterOnlyMode?.addEventListener('change', async () => {
  const nextEnabled = Boolean(inputRegisterOnlyMode.checked);
  inputRegisterOnlyMode.disabled = true;
  try {
    const response = await chrome.runtime.sendMessage({
      type: 'SET_REGISTER_ONLY_MODE',
      source: 'sidepanel',
    ...
    syncLatestState({ registerOnlyMode: Boolean(response.registerOnlyMode) });
    inputRegisterOnlyMode.checked = Boolean(response.registerOnlyMode);
    showToast(response.registerOnlyMode ? '已开启仅注册模式' : '已关闭仅注册模式', 'success', 1800);
  } catch (err) {
    inputRegisterOnlyMode.checked = !nextEnabled;
    syncLatestState({ registerOnlyMode: !nextEnabled });
    showToast(err.message, 'error');
  } finally {
    inputRegisterOnlyMode.disabled = isAutoRunLockedPhase() || isAutoRunScheduledPhase();
    updateButtonStates();
  }
});
...
chrome.runtime.onMessage.addListener((message) => {
  switch (message.type) {
    ...
    case 'DATA_UPDATED': {
      ...
      if (message.payload.registerOnlyMode !== undefined && inputRegisterOnlyMode) {
        inputRegisterOnlyMode.checked = Boolean(message.payload.registerOnlyMode);
        updateButtonStates();
      }
