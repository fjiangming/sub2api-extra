...
const {
  findLoopbackCallbackUrl = () => null,
  isLoopbackCallbackUrl = () => false,
} = globalThis.MultiPageOAuthStepHelpers || {};

const {
  getStep4FilterAfterTimestamp = (_state, fallbackTimestamp = 0) => fallbackTimestamp,
  getStep7FilterAfterTimestamp = (_state, fallbackTimestamp = 0) => fallbackTimestamp,
} = globalThis.MultiPageVerificationTiming || {};

const {
  buildStep3ExecutionPlan = (state = {}) => ({
    email: String(state.email || '').trim(),
    password: String(state.customPassword || '').trim(),
    shouldActivateSignupTab: true,
  }),
} = globalThis.MultiPageStep3Flow || {};

const {
  getMailReturnBehaviorAfterResend = () => ({ mode: 'activate', reloadIfSameUrl: false }),
} = globalThis.MultiPageVerificationMailReturn || {};

const {
  shouldBypassStep9 = () => false,
} = globalThis.MultiPageStep9Bypass || {};
...
const PERSISTED_SETTING_DEFAULTS = {
  ...
  registerOnlyMode: false, // 仅注册模式：完成步骤 5 后跳过后续回调流程。
  skipStep9Enabled: false, // 是否持续跳过步骤 9，适用于多轮自动运行。
  ...
  mailProvider: '163', // 验证码邮箱来源（2925 / 163 / 163-vip / qq / inbucket）。
...
const DEFAULT_STATE = {
  ...
  accounts: [], // 已注册账号记录：{ email, password, status, registeredAt, callbackAt }。
  ...
  step3StartTime: null, // 最近一次开始执行步骤 3 的时间戳，供步骤 4 过滤新邮件使用。
  step6StartTime: null, // 最近一次开始执行步骤 6 的时间戳，供步骤 7 过滤新邮件使用。
  skipStep9Enabled: false, // 是否持续跳过步骤 9。
...
async function getPersistedRegisteredAccounts() {
  const stored = await chrome.storage.local.get([REGISTERED_ACCOUNTS_STORAGE_KEY]);
  return normalizeRegisteredAccounts(stored[REGISTERED_ACCOUNTS_STORAGE_KEY]);
}

async function getState() {
  const [state, persistedSettings, persistedAccounts] = await Promise.all([
    chrome.storage.session.get(null),
    getPersistedSettings(),
    getPersistedRegisteredAccounts(),
  ]);
  const sessionAccounts = Array.isArray(state.accounts) && state.accounts.length ? state.accounts : null;
  const accounts = normalizeRegisteredAccounts(sessionAccounts || persistedAccounts);
  return { ...DEFAULT_STATE, ...persistedSettings, ...state, accounts };
}
...
async function resetState() {
  ...
  const [prev, persistedSettings, persistedAccounts] = await Promise.all([
  ...
  await chrome.storage.session.set({
    ...
    accounts: normalizeRegisteredAccounts(Array.isArray(prev.accounts) && prev.accounts.length ? prev.accounts : persistedAccounts),
...
function normalizeRegisteredAccountStatus(status) {
  return status === ACCOUNT_STATUS_CALLBACK_SUCCESS
    ? ACCOUNT_STATUS_CALLBACK_SUCCESS
    : ACCOUNT_STATUS_REGISTERED;
}

function getRegisteredAccountStatusRank(status) {
  return normalizeRegisteredAccountStatus(status) === ACCOUNT_STATUS_CALLBACK_SUCCESS ? 2 : 1;
}
...
function normalizeRegisteredAccount(account = {}) {
  const email = String(account.email || '').trim();
  if (!email) return null;

  const status = normalizeRegisteredAccountStatus(account.status);
  const createdAt = normalizeAccountDate(account.createdAt || account.registeredAt, new Date().toISOString());
  const registeredAt = normalizeAccountDate(account.registeredAt || account.createdAt, createdAt);
  const callbackAt = normalizeAccountDate(account.callbackAt, '');
  const updatedAt = normalizeAccountDate(account.updatedAt || callbackAt || registeredAt || createdAt, createdAt);

  return {
    id: String(account.id || email.toLowerCase()),
    email,
    ...
    callbackAt,
    updatedAt,
    localhostUrl: String(account.localhostUrl || ''),
  };
}

function mergeRegisteredAccountRecords(existing, incoming) {
  if (!existing) return incoming;

  const status = getRegisteredAccountStatusRank(incoming.status) >= getRegisteredAccountStatusRank(existing.status)
    ? incoming.status
    : existing.status;

  return normalizeRegisteredAccount({
    ...existing,
    ...incoming,
    ...
    registeredAt: existing.registeredAt || incoming.registeredAt,
    callbackAt: incoming.callbackAt || existing.callbackAt,
    updatedAt: incoming.updatedAt || existing.updatedAt,
    localhostUrl: incoming.localhostUrl || existing.localhostUrl,
  });
}

function normalizeRegisteredAccounts(accounts) {
  if (!Array.isArray(accounts)) return [];

  const deduped = new Map();
  for (const account of accounts) {
    const normalized = normalizeRegisteredAccount(account);
    if (!normalized) continue;
    const key = normalized.email.toLowerCase();
    deduped.set(key, mergeRegisteredAccountRecords(deduped.get(key), normalized));
  }

  return [...deduped.values()].sort((a, b) => {
    const bTime = Date.parse(b.updatedAt || b.callbackAt || b.registeredAt || b.createdAt) || 0;
    const aTime = Date.parse(a.updatedAt || a.callbackAt || a.registeredAt || a.createdAt) || 0;
    return bTime - aTime;
  });
}

async function setPersistedRegisteredAccounts(accounts) {
  if (typeof chrome === 'undefined' || !chrome.storage?.local?.set) return;
  await chrome.storage.local.set({ [REGISTERED_ACCOUNTS_STORAGE_KEY]: accounts });
}

async function syncRegisteredAccounts(accounts) {
  const normalized = normalizeRegisteredAccounts(accounts);
  await Promise.all([
    setState({ accounts: normalized }),
    setPersistedRegisteredAccounts(normalized),
  ]);
  broadcastDataUpdate({ accounts: normalized });
  return normalized;
}

async function upsertRegisteredAccount(input = {}) {
  ...
  const accounts = normalizeRegisteredAccounts(state.accounts);
  const existing = accounts.find((account) => account.email.toLowerCase() === email.toLowerCase()) || null;
  const now = new Date().toISOString();
  const requestedStatus = normalizeRegisteredAccountStatus(input.status);
  const status = getRegisteredAccountStatusRank(requestedStatus) >= getRegisteredAccountStatusRank(existing?.status)
    ? requestedStatus
    : existing.status;

  const incoming = normalizeRegisteredAccount({
    ...(existing || {}),
    ...input,
    id: existing?.id || input.id || email.toLowerCase(),
    email,
    password: input.password || existing?.password || '',
    status,
    createdAt: existing?.createdAt || input.createdAt || now,
    registeredAt: existing?.registeredAt || input.registeredAt || now,
    callbackAt: requestedStatus === ACCOUNT_STATUS_CALLBACK_SUCCESS
      ? input.callbackAt || existing?.callbackAt || now
      : existing?.callbackAt || input.callbackAt || '',
    updatedAt: now,
  });

  const nextAccounts = existing
    ? accounts.map((account) => (account.email.toLowerCase() === email.toLowerCase() ? incoming : account))
    : [...accounts, incoming];
  const synced = await syncRegisteredAccounts(nextAccounts);
  return synced.find((account) => account.email.toLowerCase() === email.toLowerCase()) || incoming;
}

async function markCurrentRegisteredAccountStatus(status, payload = {}) {
  const state = await getState();
  const email = String(payload.email || state.email || '').trim();
  if (!email) return null;

  const account = await upsertRegisteredAccount({
    email,
    password: payload.password || state.password || '',
    status,
    localhostUrl: payload.localhostUrl || state.localhostUrl || '',
  });
...
function matchesSourceUrlFamily(source, candidateUrl, referenceUrl) {
  ...
  switch (source) {
    ...
    case 'mail-2925':
      return candidate.hostname === 'www.2925.com';
    case 'qq-mail':
...
function isRegisterOnlyMode(state = {}) {
  return Boolean(state?.registerOnlyMode);
}

function isRegisterOnlySkippedStep(step, state = {}) {
  return isRegisterOnlyMode(state) && step > REGISTER_ONLY_LAST_STEP && step <= FLOW_LAST_STEP;
}

async function markRegisterOnlyRemainingStepsSkipped(options = {}) {
  const state = options.state || await getState();
  const statuses = { ...(state.stepStatuses || {}) };
  const skippedSteps = [];

  for (let step = REGISTER_ONLY_LAST_STEP + 1; step <= FLOW_LAST_STEP; step++) {
    const status = statuses[step] || 'pending';
    if (status === 'running' || isStepDoneStatus(status)) {
      continue;
    }
  ...
  if (skippedSteps.length) {
    await addLog(`仅注册模式已跳过步骤 ${skippedSteps.join('、')}。`, 'warn');
  }
...
async function requestBypassStep9(enabled) {
  const state = await ensureManualInteractionAllowed('切换跳过第 9 步');
  if (state.stepStatuses?.[9] === 'running') {
    throw new Error('步骤 9 正在运行中，当前不能切换该开关。');
  }

  const nextEnabled = Boolean(enabled);
  await setPersistentSettings({ skipStep9Enabled: nextEnabled });
  await setState({ skipStep9Enabled: nextEnabled });
  broadcastDataUpdate({ skipStep9Enabled: nextEnabled });
  await addLog(
    nextEnabled
      ? '已开启持续跳过步骤 9：后续手动执行与自动多轮都会在步骤 8 后跳过步骤 9。'
      : '已关闭持续跳过步骤 9：后续流程将恢复执行步骤 9。',
    'warn'
  );

  if (nextEnabled && isStepDoneStatus(state.stepStatuses?.[8]) && !isStepDoneStatus(state.stepStatuses?.[9] || 'pending')) {
    await setStepStatus(9, 'skipped');
    await addLog('步骤 8 已完成，步骤 9 已按持续开关自动跳过。', 'warn');
  }

  return { ok: true, skipStep9Enabled: nextEnabled };
}

async function requestRegisterOnlyMode(enabled) {
  const state = await ensureManualInteractionAllowed('切换仅注册模式');
  const nextEnabled = Boolean(enabled);

  await setPersistentSettings({ registerOnlyMode: nextEnabled });
  await setState({ registerOnlyMode: nextEnabled });
  broadcastDataUpdate({ registerOnlyMode: nextEnabled });
  await addLog(
    nextEnabled
      ? '已开启仅注册模式：流程将在步骤 5 完成后跳过步骤 6-9。'
      : '已关闭仅注册模式：后续流程可继续执行步骤 6-9。',
    'warn'
  );

  if (nextEnabled && isStepDoneStatus(state.stepStatuses?.[REGISTER_ONLY_LAST_STEP])) {
    await markRegisterOnlyRemainingStepsSkipped({
      state: {
        ...state,
  ...
  return { ok: true, registerOnlyMode: nextEnabled };
}
...
async function handleMessage(message, sender) {
  switch (message.type) {
    ...
    case 'SAVE_SETTING': {
      ...
      if (updates.registerOnlyMode) {
        const latestState = await getState();
        if (isStepDoneStatus(latestState.stepStatuses?.[REGISTER_ONLY_LAST_STEP])) {
          await markRegisterOnlyRemainingStepsSkipped({ state: latestState });
        }
      }
      return { ok: true };
    }

    case 'SET_REGISTER_ONLY_MODE': {
      return await requestRegisterOnlyMode(message.payload?.enabled);
    }
    ...
    case 'BYPASS_STEP9': {
      return await requestBypassStep9(message.payload?.enabled);
    }
...
async function handleStepData(step, payload) {
  switch (step) {
    ...
    case 4:
      ...
      break;
    case 5: {
      await markCurrentRegisteredAccountStatus(ACCOUNT_STATUS_REGISTERED, payload || {});
      const latestState = await getState();
      if (isRegisterOnlyMode(latestState)) {
        await markRegisterOnlyRemainingStepsSkipped({ state: latestState });
      }
      break;
    }
    ...
    case 8:
      ...
      if (typeof markCurrentRegisteredAccountStatus === 'function') {
        await markCurrentRegisteredAccountStatus(ACCOUNT_STATUS_REGISTERED, {
          localhostUrl: payload.localhostUrl || '',
        });
      }
      {
        const latestState = await getState();
        const step9Status = latestState.stepStatuses?.[9] || 'pending';
        if (shouldBypassStep9(latestState) && !isStepDoneStatus(step9Status)) {
          await setStepStatus(9, 'skipped');
          await addLog('步骤 8 完成后，已按持续开关自动跳过步骤 9。', 'warn');
        }
      }
      break;
    case 9: {
      ...
      const latestState = await getState();
      if (typeof markCurrentRegisteredAccountStatus === 'function') {
        await markCurrentRegisteredAccountStatus(ACCOUNT_STATUS_CALLBACK_SUCCESS, {
          localhostUrl: payload.localhostUrl || latestState.localhostUrl || '',
        });
      }
...
async function executeStep(step) {
  ...
  if (isRegisterOnlySkippedStep(step, state)) {
    await setStepStatus(step, 'skipped');
    await addLog(`步骤 ${step} 已按仅注册模式跳过。`, 'warn');
    notifyStepComplete(step, {});
    return;
  }

  if (step === 9 && shouldBypassStep9(state)) {
    await setStepStatus(9, 'skipped');
    await addLog('步骤 9 已按持续开关自动跳过。', 'warn');
    notifyStepComplete(9, {});
    return;
  }
...
async function fetch2925ChildEmail(options = {}) {
  ...
  const mainMailbox = knownMainMailbox || await fetch2925MainEmailFromPage({ activate: options.activate });
...
async function fetchEmailForCurrentProvider(options = {}) {
  const state = await getState();
  const provider = options.provider || state.mailProvider;

  if (provider === '2925') {
    return fetch2925ChildEmail(options);
  }

  return fetchGeneratedEmail(state, options);
}
...
async function ensureAutoEmailReady(targetRun, totalRuns, attemptRuns) {
  ...
  if (currentState.mailProvider === '2925') {
    const providerLabel = '2925 邮箱';
    let lastProviderError = null;
    for (let providerAttempt = 1; providerAttempt <= EMAIL_FETCH_MAX_ATTEMPTS; providerAttempt++) {
      try {
        if (providerAttempt > 1) {
          await addLog(`${providerLabel}：正在进行第 ${providerAttempt}/${EMAIL_FETCH_MAX_ATTEMPTS} 次自动获取重试...`, 'warn');
        }
        const preparedEmail = await fetch2925ChildEmail({ generateNew: true });
        await addLog(`=== 目标 ${targetRun}/${totalRuns} 轮：${providerLabel}已就绪：${preparedEmail}（第 ${attemptRuns} 次尝试，第 ${providerAttempt}/${EMAIL_FETCH_MAX_ATTEMPTS} 次获取）===`, 'ok');
        return preparedEmail;
      } catch (err) {
        lastProviderError = err;
        await addLog(`${providerLabel}自动获取失败（${providerAttempt}/${EMAIL_FETCH_MAX_ATTEMPTS}）：${err.message}`, 'warn');
      }
    }
    await addLog(`${providerLabel}自动获取已连续失败 ${EMAIL_FETCH_MAX_ATTEMPTS} 次：${lastProviderError?.message || '未知错误'}`, 'error');
    await addLog(`=== 目标 ${targetRun}/${totalRuns} 轮已暂停：请先自动获取邮箱或手动粘贴邮箱，然后继续 ===`, 'warn');
    await broadcastAutoRunStatus('waiting_email', { currentRun: targetRun, totalRuns, attemptRun: attemptRuns });
    await waitForResume();
...
function isStep8AddPhoneError(error) {
  const message = String(typeof error === 'string' ? error : error?.message || '');
...
async function runAutoSequenceFromStep(startStep, context = {}) {
  ...
  const STEP8_ADD_PHONE_MAX_RETRIES = 3;
  let step8AddPhoneRetries = 0;
  ...
  if (isRegisterOnlyMode(initialState) && startStep > REGISTER_ONLY_LAST_STEP) {
    await markRegisterOnlyRemainingStepsSkipped({ state: initialState });
    await addLog('仅注册模式：当前进度已超过步骤 5，本轮无需执行后续步骤。', 'ok');
    return;
  }
  ...
  while (step <= 9) {
    try {
      ...
      if (step === 5) {
        ...
        if (isRegisterOnlyMode(latestState)) {
          await markRegisterOnlyRemainingStepsSkipped({ state: latestState });
          await addLog('仅注册模式：步骤 5 已完成，本轮流程到此结束。', 'ok');
          break;
        }
      ...
      // ★ Step 8 add phone 检测 → 回退到 step 6 刷新 OAuth 会话重试
      const shouldRetryStep8AddPhone = step === 8
        && isStep8AddPhoneError(err)
        && step8AddPhoneRetries < STEP8_ADD_PHONE_MAX_RETRIES;

      if (shouldRetryStep8AddPhone) {
        step8AddPhoneRetries += 1;
        await addLog(
          `步骤 8：add phone 未自动跳过，回到步骤 6 刷新 OAuth 会话重试（${step8AddPhoneRetries}/${STEP8_ADD_PHONE_MAX_RETRIES}）...`,
          'warn'
        );
        await invalidateDownstreamAfterStepRestart(6, {
          logLabel: `步骤 8 add phone 后回退到步骤 6 重试（${step8AddPhoneRetries}/${STEP8_ADD_PHONE_MAX_RETRIES}）`,
        });
        step = 6;
        continue;
      }
  ...
  while (successfulRuns < totalRuns && attemptRuns < maxAttempts) {
    ...
    if (!useExistingProgress) {
      ...
      const keepSettings = {
        ...
        registerOnlyMode: prevState.registerOnlyMode,
...
async function executeStep3(state) {
  ...
  } else if (!resolvedEmail && state.mailProvider === '2925') {
    resolvedEmail = await fetch2925ChildEmail({ generateNew: true });
  }
  ...
  await setState({ step3StartTime: Date.now() });
  const plan = buildStep3ExecutionPlan({
    email: resolvedEmail,
    customPassword: state.customPassword,
  });
  const password = plan.password || state.customPassword || generatePassword();
  ...
  if (plan.shouldActivateSignupTab) {
    let signupTabId = await getTabId('signup-page');
    if (signupTabId) {
      await chrome.tabs.update(signupTabId, { active: true });
    } else if (state.oauthUrl) {
      signupTabId = await reuseOrCreateTab('signup-page', state.oauthUrl);
    } else {
      throw new Error('注册页标签页不存在，且缺少 OAuth 链接，无法继续步骤 3。');
    }
    await addLog('步骤 3：已先切回注册页，再开始填写邮箱和密码。', 'info');
...
async function executeStep4(state) {
  ...
  const filterAfterTimestamp = getStep4FilterAfterTimestamp(state, stepStartedAt);
  const signupTabId = await getTabId('signup-page');
  if (!signupTabId) {
...
async function runStep7Attempt(state) {
  ...
  if (mail.error) throw new Error(mail.error);
  const stepStartedAt = Date.now();
  const filterAfterTimestamp = getStep7FilterAfterTimestamp(state, stepStartedAt);
...
async function waitForStep8Ready(tabId, timeoutMs = STEP8_READY_WAIT_TIMEOUT_MS) {
  const ADD_PHONE_AUTO_SKIP_WAIT_MS = 15000;
  const start = Date.now();
  let recovered = false;
  let addPhoneSeenAt = 0;

  while (Date.now() - start < timeoutMs + ADD_PHONE_AUTO_SKIP_WAIT_MS) {
    throwIfStopped();
    const pageState = await getStep8PageState(tabId);
    if (pageState?.addPhonePage) {
      if (!addPhoneSeenAt) {
        addPhoneSeenAt = Date.now();
        await addLog('步骤 8：检测到 add phone 页面，等待 OpenAI 自动跳过（最多 15 秒）...', 'warn');
      }
      if (Date.now() - addPhoneSeenAt > ADD_PHONE_AUTO_SKIP_WAIT_MS) {
        throw new Error('步骤 8：认证页长时间停留在手机号页面（add phone），OpenAI 未自动跳过，无法继续。');
      }
      await sleepWithStop(500);
      continue;
    }
    if (addPhoneSeenAt) {
      await addLog('步骤 8：add phone 页面已被 OpenAI 自动跳过，继续等待 OAuth 同意页...', 'ok');
      addPhoneSeenAt = 0;
    }
...
async function waitForStep8ClickEffect(tabId, baselineUrl, timeoutMs = STEP8_CLICK_EFFECT_TIMEOUT_MS) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    const pageState = await getStep8PageState(tabId);
    if (pageState?.addPhonePage) {
      // add phone 页面出现，但 OpenAI 可能会自动跳过，不立即抛错
      // webNavigation 监听器仍在后台运行，如果 OpenAI 自动重定向到 localhost 会自动捕获
      await addLog('步骤 8：点击"继续"后页面进入了手机号页面，等待 OpenAI 可能的自动跳过...', 'warn');
      await sleepWithStop(500);
      continue;
    }
    if (pageState === null) {
