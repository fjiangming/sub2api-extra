...
async function findContinueButton(timeout = 10000) {
  ...
  while (Date.now() - start < timeout + ADD_PHONE_AUTO_SKIP_WAIT_MS) {
    throwIfStopped();

    if (isAddPhonePageReady()) {
      if (!addPhoneSeenAt) {
        addPhoneSeenAt = Date.now();
        log('步骤 8：检测到 add phone 页面，等待 OpenAI 自动跳过（最多 15 秒）...');
      }
      if (Date.now() - addPhoneSeenAt > ADD_PHONE_AUTO_SKIP_WAIT_MS) {
        throw new Error('步骤 8：认证页长时间停留在手机号页面（add phone），OpenAI 未自动跳过，无法继续。URL: ' + location.href);
      }
      await sleep(500);
      continue;
    }

    // add phone 消失了，OpenAI 已自动跳过
    if (addPhoneSeenAt) {
      log('步骤 8：add phone 页面已被 OpenAI 自动跳过，继续查找"继续"按钮...');
      addPhoneSeenAt = 0;
    }
