...
| `PORT` | ❌ | `3100` | 本服务监听端口 |
| `ACCOUNT_MANAGER_BIND_HOST` | ❌ | `127.0.0.1` | 本地运行监听地址；容器内由 Compose 覆盖 |
...
4. 启动服务：
   ...
   ```
   服务默认仅映射到宿主机 `127.0.0.1:9870`，需要通过本机反向代理对外提供域名访问。
...
4. 填写如下信息：
   - **标签 (Label)**: 账号管理 (或我的账号)
   - **URL**: `https://account.example.com`（填写反向代理后的 HTTPS 域名）
